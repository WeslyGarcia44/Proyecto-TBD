...
def add_game_to_user_list(request, game_id):
    ...
    if created:
        pass
    return redirect(
        'alguna_vista_para_ver_la_lista')  # Redirige a la vista donde el usuario puede ver su lista de juegos
...
def Games_(request):
    ...
    return render(request, 'registration/library/games.html', {'games': games})


# def tu_vista(request):
   # games = Game.objects.all()  # Obtiene todos los juegos
   # return render(request, 'registration/library/games.html', {'games': games})
